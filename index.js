...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.dhgbbnz.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        const allCategoryCollection = client.db('resellingYourCamera').collection('allCategory');
        const allProductsCollection = client.db('resellingYourCamera').collection('allProducts');
        const usersCollection = client.db('resellingYourCamera').collection('users');
        const ordersCollection = client.db('resellingYourCamera').collection('myOrders');
